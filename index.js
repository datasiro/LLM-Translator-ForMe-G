...
const extensionName = "LLM-Translator-ForMe-G";
...
async function callLLMAPI(fullPrompt) {
    ...
    switch (provider) {
        ...
        case 'google':
            ...
            break;
        case 'vertexai': // [추가] 이 case 블록
            // Vertex AI는 express 모드(API 키) 또는 full 모드(서비스 계정)를 사용합니다.
            // 백엔드가 둘 다 처리하므로, 여기서는 둘 중 하나(API 키)만 검사합니다.
            apiKey = secret_state[SECRET_KEYS.VERTEXAI] || secret_state[SECRET_KEYS.VERTEXAI_SERVICE_ACCOUNT];
            chatCompletionSource = 'vertexai'; // 백엔드에서 'vertexai'를 인식해야 함
            break;
        case 'cohere':
    ...
    if (extensionSettings.llm_prefill_toggle) {
        ...
        const role = (provider === 'google' || provider === 'vertexai') ? 'model' : 'assistant'; // [수정]
    ...
    // [추가] Vertex AI를 "Full Version" (서비스 계정 모드)로 강제합니다.
...
class PresetManager {
    constructor() {
        this.presets = [];
        this.storageKey = 'llmTranslatorPresets'; // localStorage에 저장될 키
        this.loadFromLocalStorage();
        this.initializeEventListeners(); // 이벤트 리스너도 여기서 초기화
    }
    ...
    loadFromLocalStorage() {
        ...
        if (saved) {
            try {
                ...
                console.error("LLM Translator: 프리셋 로드 실패", e);
                this.presets = [];
            }
        }
    }
    ...
    initializeEventListeners() {
        ...
        // '불러오기' 버튼 클릭 시
        $(document).off('click', '#llm_preset_load').on('click', '#llm_preset_load', () => {
    ...
    // HTML의 <select> 드롭다운 메뉴를 업데이트
    updatePresetDropdown() {
        const select = $('#llm_preset_select');
        if (!select.length) return; // UI가 아직 없으면 중단

        select.html('<option value="">-- 프리셋 선택 --</option>'); // 기본 옵션
    ...
    async saveCurrentPreset() {
        ...
        // 1. 'extensionSettings'의 현재 상태를 깊은 복사
        const currentSettings = simpleDeepClone(extensionSettings);

        // 2. 커스텀 프롬프트도 함께 저장
        const currentCustomPrompts = JSON.parse(localStorage.getItem('customPrompts') || '[]');

        const newPreset = {
            id: `preset_${Date.now()}`,
            name: presetName,
            settings: currentSettings, // (1) extensionSettings
            customPrompts: currentCustomPrompts // (2) 커스텀 프롬프트
        };

        this.presets.push(newPreset);
        this.saveToLocalStorage(); // 로컬 스토리지에 저장
        this.updatePresetDropdown(); // 드롭다운 UI 새로고침
        
        // 새로 저장된 프리셋을 선택 상태로 만듦
    ...
    async loadSelectedPreset() {
        ...
        // 1. 커스텀 프롬프트 복원 (로컬 스토리지에 덮어쓰기)
        localStorage.setItem('customPrompts', JSON.stringify(loadedCustomPrompts));
        if (promptManager) {
            promptManager.loadFromLocalStorage(); // PromptManager가 새 데이터를 다시 읽고 UI(드롭다운)를 업데이트하도록 함
        }
        
        // 2. extensionSettings 복원 (기존 설정 지우고 새 설정 적용)
        ...
        // 3. UI 새로고침
        loadSettings(); // extensionSettings를 기반으로 모든 UI(체크박스, 모델 등)를 다시 그림
        loadSelectedPrompt(); // ★★★ '프롬프트 수정' 편집창의 내용을 업데이트 (이게 빠졌었음)

        // 4. 프리셋 드롭다운 상태 유지
        this.updatePresetDropdown();
        $('#llm_preset_select').val(selectedId); // 방금 불러온 프리셋 선택

        saveSettingsDebounced(); // 불러온 설정을 현재 설정으로 저장
        toastr.success(`프리셋 "${preset.name}"을(를) 불러왔습니다.`);
    }
    ...
    async deleteSelectedPreset() {
        ...
        this.presets = this.presets.filter(p => p.id !== selectedId); // 배열에서 제거
        this.saveToLocalStorage(); // 변경사항 저장
        this.updatePresetDropdown(); // UI 새로고침
